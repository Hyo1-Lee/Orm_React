									...
									<Link to='#' onClick={(e) => openUserChat(e, chat)}>
										<div className='d-flex'>
											{chat.category_code === 2 ? (
												...
												<div className={"chat-user-img " + chat.status + " align-self-center me-1 ms-0"}>
													<img src={chat.channel_img_path} className='rounded-circle avatar-xs' alt='chatvia' />
													{chat.status && <span className='user-status'></span>}
												</div>
											)}

											<div className='flex-grow-1 overflow-hidden'>
												<h5 className='text-truncate font-size-15 mb-1 ms-3'>{chat.channel_name}</h5>
											...
											<div className='font-size-11'>{chat.message.length > 0 ? chat.msg_date : null}</div>
											{chat.unRead === 0 ? null : (
